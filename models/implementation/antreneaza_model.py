...
def vizualizeaza_rezultate(y_test, y_pred, X_test, model):
    ...
    plt.savefig( os.path.join(MODEL_DIR, "matrice_confuzie.png") , dpi=150)
...
def salveaza_model(model):
    ...
    nume_fisier = os.path.join(SAVE_MODEL_DIR, f'model_v1_{timestamp}.pkl')  
    ...
    joblib.dump(model, os.path.join(SAVE_MODEL_DIR, 'model_latest.pkl') )
